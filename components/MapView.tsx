...
const mockRoutePath = [
  districtCoordinates["san isidro"],
  districtCoordinates["miraflores"],
  districtCoordinates["san borja"],
];
...
const fetchRoute = async (origin: string, destination: string) => {
  return new Promise<{ path: Array<{ lat: number; lng: number }> }>((resolve) => {
    setTimeout(() => {
      resolve({ path: mockRoutePath });
    }, 100);
  });
};
...
export default function MapView({ origin, destination }: MapViewProps) {
  ...
  return (
    <div className="w-full h-[60vh] rounded-xl border border-[#2a2a2a] overflow-hidden shadow-lg">
      ...
      >
        <Marker
          position={districtCoordinates["san isidro"]}
          icon={{
        ...
        />
        {originCoords && originCoords.lat !== districtCoordinates["san isidro"].lat && (
          <Marker
